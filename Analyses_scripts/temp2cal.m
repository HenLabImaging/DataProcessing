...
for i=1:length(zoneXtoY(1:k))
    Close2open_cell1(i,1)=sum(abs((Aligned_transients(zoneXtoY(i)-50:zoneXtoY(i),ii))));
    Close2open_cell1(i,2)=sum(abs(Aligned_transients(zoneXtoY(i):zoneXtoY(i)+50,ii)));
end
...
figure(1);subplot(kk,kk,k-1);compass(meanClose2openCell_norm(:,1),meanClose2openCell_norm(:,2),'b')
