...
Image_dir=pwd;
     ...
     num_cell2=length(dir([image_dir2 imagepath '*' format]));
...
subplot(2,3,3);histogram(min_intercell1);hold on;histogram(min_intercell2);
xlabel('pix','FontSize',16);ylabel('#cells','FontSize',16);box off
subplot(2,3,5);histogram(min_intercell1);hold on;histogram(min_intercell2);
xlabel('pix','FontSize',16);ylabel('#cells','FontSize',16);box off
...
bb(i)=b
    ...
    cell=imread([image_dir2 imagepath num2str(ii) format]);
...
%               cell=imread([Image_dir1,'/ic',num2str(matched_cells(i,1)-1),'.tif']);
    ...
    for i=1:length(matched_cells)
        if(matched_cells(i,2)==1)
            cell=imread([image_dir2 imagepath num2str(i) format]);
        else
              cell=imread([image_dir2 imagepath num2str(matched_cells(i,2)-1) format]);
        end
        Comb_cell=imadd(Comb_cell,cell);
