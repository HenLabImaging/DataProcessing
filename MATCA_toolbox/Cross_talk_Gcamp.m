...
subplot(2,3,2);imagesc(normxy1);axis tight;colormap gray;title('Inter-cell Centoids','FontSize',16);colorbar
subplot(2,3,3);imagesc(CC);axis tight;colormap gray;title('Temporal Correlation','FontSize',16);colorbar
caxis([0 0.5])    
subplot(2,3,4);imagesc(CCC);title('False correlation','FontSize',16);colorbar

subplot(2,3,5);hold on;set(gca,'FontSize',15);title('Spatio-temporal Regression');xlabel('cell');ylabel('Spearman R');
for i=1:length(CC)
plot(xy1(i,:),CC(i,:),'b.')
% if(any(xy1(i,:)<10))
...
axis tight
   ...
   %% ColorCoding the 
      ...
      ccc(ccc>0)=10;
   Thresh=0.05;
         Thresh=str2double(char(inputdlg('Enter the coeff threshold:')));
   [Xx,Yy]=find(CCC>Thresh);
   subplot(2,3,6);image(ccc)
   ss=[];
     for i=1:length(Xx)
    ss(i)=xy1(Xx(i),Yy(i));
    end
    prc=prctile(ss,[95])
    length(find(ss<10))/length(ss)*100
    
   %%
   for ij=1:length(Xx);
   cell1=imread([image_dir imagepath num2str(Xx(ij)-1) format]);
   cell2=imread([image_dir imagepath num2str(Yy(ij)-1) format]);
    coeff_color=10*CCC(Xx(ij),Yy(ij));
   
    hold on;contour(cell1*coeff_color);hold on;contour(cell2*coeff_color)
   end
   colormap jet(20)
    %%
