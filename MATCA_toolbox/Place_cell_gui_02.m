function Place_cell_gui_02
Place.fh = figure('Visible','on','Position',[20,20,1500,1000],'Units','normalized');
set(gcf,'Name','PlaceCellAnalysis v0.2','Color',[0.98 0.98 0.99],'NumberTitle','off')
...
uicontrol('Style','Text','String','Bin Size','Position',[150,660,100,30],'FontSize',14,'backgroundc',get(Place.fh,'color'),'Units','normalized');
 ...
 Place.threshold = uicontrol('Style',' edit','String','  ','Position',[130,460,30,20],'FontSize',16,'Units','normalized');
...
% uicontrol('Style','Text','String','Draw Trajectory','Position',[330,660,100,25],'FontSize',12,'Units','normalized');
 ...
 Place.checkbox1_5=uicontrol('Style','checkbox','String','See Behavior','Position',[260,630,100,30],'background','w','HorizontalAlign','left','FontSize',12,'Units','normalized');
  ...
  Place.cellInterest  = uicontrol('Style',' edit','String','Interested Cells','Position',[400,520,200,30],'FontSize',16,'Units','normalized');
  ...
  Place.Analyze = uicontrol('Parent', Place.fh ,'Style','togglebutton','String','Raw Place Fields','Position',[150,500,220,80],'Callback',@analyze,'FontSize',16,'Units','normalized');
   ...
   Place.checkbox1_9=uicontrol('Style','checkbox','String','Store','Position',[30,420,100,20],'background','w','HorizontalAlign','left','FontSize',12,'Units','normalized');
      Place.checkbox1_16=uicontrol('Style','checkbox','String','Threshold =','Position',[30,460,100,20],'background','w','HorizontalAlign','left','FontSize',12,'Units','normalized');
          Place.checkbox1_17=uicontrol('Style','checkbox','String','Overlap','Position',[30,500,100,20],'background','w','HorizontalAlign','left','FontSize',12,'Units','normalized');
...
Place.checkbox1_12=uicontrol('Style','checkbox','String','Plot','Position',[40,220,100,20],'background','w','HorizontalAlign','left','FontSize',12,'Units','normalized');
 ...
 Place.bin2= uicontrol('Style',' edit','String','bin size','Position',[1050,770,100,30],'FontSize',16,'Units','normalized');
...
hold on;scatter(xxyy(:,1),xxyy(:,2),'bo','LineWidth',1);axis tight
    ...
    function analyze(hObject, eventdata)
            object_data=guidata(hObject);
            raw=object_data.raw;
            if(Place.checkbox1_16.Value == 1)
            thresh=str2double(get(Place.threshold,'str'));
            [time,cells]=find(raw>thresh);
            raw_thresh=[];
            for i=1:min(size(raw))
                if(any(cells==i))
                raw_thresh=[raw_thresh i];
                else
                    raw(i,:)=0;
                end
            end
...
if(Place.checkbox1_8.Value == 1)
    if(Place.checkbox1_17.Value == 1)
        figure(14);hold on;subplot(xk,xk,kkk);hold on;contour(norm_map);axis xy,colormap(jet(10));caxis([0 1]);title(['Cell',num2str(kkk)],'Fontsize',5);
    else
...
function analyze2(hObject, eventdata)
            ...
            figure;
            for ik=1:ll
                subplot(lll,lll,ik);pcolor(squeeze(bigMap(Cells_interest(ik),:,:)));axis xy,colormap(jet(50));caxis([0 1]);
...
%      object_data=guidata(hObject);
    ...
    if(isnan(str2double(get(Place.bin2,'str'))))
        msgbox('Enter bin size','Error Check','Error');
    else
        bin_size=str2double(get(Place.bin2,'str'));
    end
...
function AnalyzeCorrData(hObject, eventdata)
        ...
        shuffleN=str2num(char(inputdlg('Enter the shuffling repetition:')))

        
        for kkk=1:length(Map1)%Place fields for set of #N Cells
                sliding_win=moving_window;
        if(sliding_win>0)
%             cc=[];
%             c=[];
               map11=squeeze(Map1(kkk,:,:));
             map12=squeeze(Map2(kkk,:,:));

            shift1=circshift(map12,[sliding_win,0]);
            shift2=circshift(map12,[-sliding_win,0]);
            shift3=circshift(map12,[0,sliding_win]);
            shift4=circshift(map12,[0,-sliding_win]);
            
            corrcell1=corr(map11,shift1);
            corrcell2=corr(map11,shift2);
            corrcell3=corr(map11,shift3);
            corrcell4=corr(map11,shift4);
            cc1=corrcoef(map11,shift1);
            cc2=corrcoef(map11,shift2);
            cc3=corrcoef(map11,shift3);
            cc4=corrcoef(map11,shift4);
            AA=max([cc1(1,2) cc2(1,2) cc3(1,2) cc4(1,2)]);
...
[m,n]=size(map12);
N=m*n;
newmatrix=reshape(map12,[1,N]);

for ii=1:shuffleN
%     aaa=[];
    a=[];aa=[];
for i=1:m*n
    tempmap12(i)=newmatrix(randi(N));
end
mapnew12=reshape(tempmap12,[m,n]);
aa=corrcoef(map11,mapnew12);
if(isnan(aa(2,1))==1)
    aa(2,1)=0;
end
aaa(ii)=aa(2,1);
...
pd=fitdist(aaa','Normal');
CI=pd.mu+pd.sigma*2;
% figure(2);hold on;plot(kkk,[AA],'ro');plot(kkk,CI,'bx')
% 
% pause(.5)      
...
% figure(10);
hold on;plot(kkk,CI,'r^','MarkerSize',5)
hold on;plot(kkk,AA,'bo','MarkerSize',5);
% pause(0.1)
...
set(gca,'FontSize',20);ylabel('Correlation');axis([0 length(Map1) 0 1 ])
...
%         set(Place.parameters,'string',['#Cells' ' ' num2str(length(CCs)) ', ' 'MeanCorr:'  '' num2str(mean(cc)) ]);
...
function reset(hObject, eventdata,handles)
       close figure 'PlaceCellAnalysis v0.2'
       Place_cell_gui_02
end
end